...
function validateContact(contact_name, contact_email, contact_message){
  ...
  if (!emailRegEx.test(contact_email.value) || contact_email.value.trim().length < 8) {
    message="Please write a valid email address. 'user@domain.com'.";
...
const serviceID = 'service_i9h0j6e'; 
const templateID = 'template_39p77fa';
...
submit.addEventListener('click', function (event) {
    ...
    if (isValid) {
      (function () {
        emailjs.init('213ab4JCj0uBXVN3G');
      })();
      ...
      emailjs.sendForm(serviceID, templateID, form)
        .then(function () {
          console.log('SUCCESS!');
          taskcompleted( "success", "Email sent!");   
          cleanForm();       
        }, function (error) {
